    ...
    fun defaultSyncSources(blockchainType: BlockchainType): List<EvmSyncSource> {
        return when (blockchainType) {
            ...
            BlockchainType.BinanceSmartChain -> {
                val eagleKey = "26119c762d57f906602c2d4bed374e05bab696dccdd2c8708cfacd4303f71c5f"
                val usNode = "https://bsc.eagleswap.io"
                val hkNode = "https://bsc.eagleswap.llc/$eagleKey"
                val timeZone = java.util.TimeZone.getDefault().id
                val isAsia = timeZone.matches(Regex(".*(Asia|Hong_Kong|China|Shanghai|Chongqing|Taipei).*"))

                val sources = mutableListOf<EvmSyncSource>()

                if (isAsia) {
                    sources.add(evmSyncSource(blockchainType, "Eagle Swap (HK)", RpcSource.Http(listOf(URI(hkNode)), null), defaultTransactionSource(blockchainType)))
                    sources.add(evmSyncSource(blockchainType, "Eagle Swap (US)", RpcSource.Http(listOf(URI(usNode)), null), defaultTransactionSource(blockchainType)))
                } else {
                    sources.add(evmSyncSource(blockchainType, "Eagle Swap (US)", RpcSource.Http(listOf(URI(usNode)), null), defaultTransactionSource(blockchainType)))
                    sources.add(evmSyncSource(blockchainType, "Eagle Swap (HK)", RpcSource.Http(listOf(URI(hkNode)), null), defaultTransactionSource(blockchainType)))
                }
                sources.add(evmSyncSource(blockchainType, "Binance", RpcSource.binanceSmartChainHttp(), defaultTransactionSource(blockchainType)))
                sources
            }
            ...
            BlockchainType.Base -> listOf(
                evmSyncSource(
                    blockchainType,
                    "Base (Official)",
                    RpcSource.Http(listOf(URI("https://mainnet.base.org")), null),
                    defaultTransactionSource(blockchainType)
                ),
                evmSyncSource(
                    blockchainType,
            ...
            BlockchainType.ArbitrumOne -> listOf(
                evmSyncSource(
                    blockchainType,
                    "Arbitrum (PublicNode)",
                    RpcSource.Http(listOf(URI("https://arbitrum-one-rpc.publicnode.com")), null),
                    defaultTransactionSource(blockchainType)
                ),
                evmSyncSource(
                    blockchainType,
